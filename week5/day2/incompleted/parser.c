...
void compileWhileSt(void)
{
  ...
  CodeAddress beginOfWhileAddress;
  Instruction* fj;

  beginOfWhileAddress = getCurrentCodeAddress();
  eat(KW_WHILE);
  compileCondition();
  fj = genFJ(DC_VALUE);
  eat(KW_DO);
  compileStatement();
  genJ(beginOfWhileAddress);
  updateFJ(fj,beginOfWhileAddress);
}
...
void compileForSt(void)
{
  ...
  // get address of l1
  beginOfFor = getCurrentCodeAddress();
  eat(KW_TO);
  type = compileExpression();
  checkTypeEquality(varType, type);
  genLE();
